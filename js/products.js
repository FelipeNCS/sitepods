...
function setupTabFilters() {
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            // Reload products based on selected model
            const selectedModel = tab.getAttribute('data-model');
            loadProducts(selectedModel);
        });
    });
}


async function loadProducts(selectedModel = 'todos') {
    ...
    try {
        ...
        if (products.length === 0) {
            grid.innerHTML = `
                <div class="empty-state" style="grid-column: 1 / -1;">
                    ...
                    <p>Nenhum pod descartável encontrado nesta categoria.</p>
                </div>
            `;
            return;
        }
        ...
        products.forEach(product => {
            ...
            // Bind click to floating cart button
        ...
        console.error('Erro ao buscar e exibir produtos', e);
        grid.innerHTML = `
            <div class="empty-state" style="grid-column: 1 / -1;">
                <p>Ocorreu um erro ao carregar os pods descartáveis. Verifique a conexão com o servidor.</p>
            </div>
        `;
    }
}
